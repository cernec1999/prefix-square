...
class SquareCC(BasePaymentProvider):
        ...
        return OrderedDict(
            [
                (
                    "first_name",
                    forms.CharField(
                        label=_("First name"), max_length=255, required=True
                    ),
                ),
                (
                    "last_name",
                    forms.CharField(
                        label=_("Last name"), max_length=255, required=True
                    ),
                ),
                (
                    "address_line_1",
                    forms.CharField(
                        label=_("Address line 1"), max_length=255, required=True
                    ),
                ),
                (
                    "address_line_2",
                    forms.CharField(
                        label=_("Address line 2"), max_length=255, required=False
                    ),
                ),
                (
                    "city",
                    forms.CharField(label=_("City"), max_length=255, required=True),
                ),
                (
                    "state",
    ...
    def checkout_prepare(
        ...
        request.session["payment_square_card_brand"] = request.POST.get(
            "square_card_brand", ""
        )
        request.session["payment_square_card_last4"] = request.POST.get(
            "square_card_last4", ""
        )
        request.session["payment_square_card_location_id"] = request.POST.get(
            "square_card_location_id", ""
        )
        request.session["payment_square_card_source_id"] = request.POST.get(
            "square_card_source_id", ""
        )
        request.session["payment_square_card_verification"] = request.POST.get(
            "square_card_verification", ""
        )
        request.session["payment_square_card_idempotency_token"] = request.POST.get(
            "square_card_idempotency_token", ""
        )
        square_card_source_id = request.POST.get("square_card_source_id", "")

        if square_card_source_id == "":
            messages.warning(
                request, _("You may need to enable JavaScript for Square payments.")
            )
            return False
        return True
    ...
    def execute_payment(self, request: HttpRequest, payment: OrderPayment):
        if not self.payment_is_valid_session(request):
            raise PaymentException(
                _(
                    "We were not able to process your credit card information. Please try again."
                )
            )
        ...
        if intent.errors and len(intent.errors) > 0:
            ...
            raise PaymentException(
                _(
                    "We were not able to process your credit card information. "
                    + error_message
                )
            )
        ...
        if (
            not intent.body["payment"]
            or intent.body["payment"]["status"] != "COMPLETED"
        ):
            ...
            raise PaymentException(
                _("We were not able to process your credit card information.")
            )
    ...
    def payment_is_valid_session(self, request):
        ...
        return request.session.get(
            "payment_square_card_brand", False
        ) and request.session.get("payment_square_card_last4", False)
